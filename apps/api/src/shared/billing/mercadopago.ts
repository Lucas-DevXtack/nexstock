...
type MpRequestInit = RequestInit & { expectOk?: boolean };

async function mpFetch<T>(path: string, init: MpRequestInit = {}): Promise<T> {
  if (!env.MP_ACCESS_TOKEN) throw new HttpError(500, 'Mercado Pago não configurado');
...
function sanitizeBackUrl(url?: string | null) {
  const v = String(url || '').trim();
...
export function amountForPlan(plan: 'PRO' | 'BUSINESS') {
...
export async function getMerchantOrder(orderId: string) {
  return mpFetch<any>(`/merchant_orders/${orderId}`, {
    method: 'GET',
  });
}