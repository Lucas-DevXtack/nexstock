...
export async function createInvite(tenantId: string, invitedByUserId: string, email: string, role: MemberRole) {
  ...
  if (pending) throw new HttpError(409, 'Already has a pending invite');
...
export async function revokeInvite(tenantId: string, inviteId: string) {
  ...
  if (inv.status !== InviteStatus.PENDING) throw new HttpError(409, 'Only pending invites can be revoked');
...
export async function acceptInvite(userId: string, token: string) {
  ...
  if (!inv || inv.status !== InviteStatus.PENDING) throw new HttpError(404, 'Invite not found');

  const user = await prisma.user.findUnique({ where: { id: userId } });
  if (!user) throw new HttpError(404, 'User not found');
  if (user.email.toLowerCase() !== inv.email.toLowerCase()) throw new HttpError(403, 'Invite email mismatch');
...
export async function transferOwnership(tenantId: string, actorUserId: string, targetUserId: string) {
  if (actorUserId === targetUserId) throw new HttpError(400, 'Escolha outro membro para receber a propriedade');
  const actor = await prisma.tenantMember.findUnique({ where: { tenantId_userId: { tenantId, userId: actorUserId } } });
  if (!actor || actor.role !== 'OWNER') throw new HttpError(403, 'Só o dono pode transferir ownership');
...
export async function removeMember(tenantId: string, actorUserId: string, targetUserId: string) {
  if (actorUserId === targetUserId) throw new HttpError(400, 'Use sair da empresa para remover a si mesmo');
  const actor = await prisma.tenantMember.findUnique({ where: { tenantId_userId: { tenantId, userId: actorUserId } } });
  if (!actor) throw new HttpError(403, 'Você não faz parte desta empresa');

  const target = await prisma.tenantMember.findUnique({ where: { tenantId_userId: { tenantId, userId: targetUserId } } });
  if (!target) throw new HttpError(404, 'Membro não encontrado');

  if (actor.role === 'STAFF') throw new HttpError(403, 'Sem permissão');
  if (actor.role === 'MANAGER' && target.role !== 'STAFF') throw new HttpError(403, 'Manager só pode remover STAFF');
  if (target.role === 'OWNER') {
    const owners = await prisma.tenantMember.count({ where: { tenantId, role: 'OWNER' } });
    if (owners <= 1) throw new HttpError(409, 'A empresa precisa manter ao menos um OWNER');
  }
...
export async function leaveCompany(tenantId: string, userId: string) {
  ...
  if (member.role === 'OWNER') {
    const owners = await prisma.tenantMember.count({ where: { tenantId, role: 'OWNER' } });
    if (owners <= 1) throw new HttpError(409, 'Transfira a propriedade antes de sair da empresa');
  }
...
export async function updateMemberRole(tenantId: string, actorUserId: string, targetUserId: string, role: MemberRole) {
  const actor = await prisma.tenantMember.findUnique({ where: { tenantId_userId: { tenantId, userId: actorUserId } } });
  if (!actor || actor.role !== 'OWNER') throw new HttpError(403, 'Só o dono pode alterar cargos');
...
export async function setMemberPermissions(tenantId: string, actorUserId: string, targetUserId: string, permissions: { key: string; allowed: boolean }[]) {
  const actor = await prisma.tenantMember.findUnique({ where: { tenantId_userId: { tenantId, userId: actorUserId } } });
  if (!actor || actor.role !== 'OWNER') throw new HttpError(403, 'Só o dono pode ajustar permissões');

  const target = await prisma.tenantMember.findUnique({ where: { tenantId_userId: { tenantId, userId: targetUserId } } });
  if (!target) throw new HttpError(404, 'Membro não encontrado');
  if (target.role === 'OWNER') throw new HttpError(409, 'Não edite permissões do OWNER por override');
  ...
  await prisma.$transaction(async (tx) => {
    ...
    if (rows.length) {
      await tx.memberPermission.createMany({
        data: rows.map((p) => ({ tenantId, userId: targetUserId, key: p.key as any, allowed: !!p.allowed })),
      });
    }
  });
...
export async function getMemberPermissions(tenantId: string, actorUserId: string, targetUserId: string) {
  const actor = await prisma.tenantMember.findUnique({ where: { tenantId_userId: { tenantId, userId: actorUserId } } });
  if (!actor) throw new HttpError(403, 'Você não faz parte desta empresa');
