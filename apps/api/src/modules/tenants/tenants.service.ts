...
async function assertOwner(tenantId: string, userId: string) {
  ...
  if (!member) throw new HttpError(403, 'Você não faz parte desta empresa');
  if (member.role !== 'OWNER') throw new HttpError(403, 'Apenas o dono pode executar esta ação');
  return member;
}

export async function createTenant(ownerUserId: string, name: string) {
  const trimmed = String(name || '').trim();
  if (trimmed.length < 2) throw new HttpError(400, 'Nome obrigatório');
...
export async function getTenantById(tenantId: string) {
  ...
  if (!tenant) throw new HttpError(404, 'Empresa não encontrada');
  return tenant;
}
...
export async function renameTenant(tenantId: string, userId: string, name: string) {
  await assertOwner(tenantId, userId);
  const trimmed = String(name || '').trim();
  if (!trimmed) throw new HttpError(400, 'Nome obrigatório');
...
export async function archiveTenant(tenantId: string, userId: string) {
  await assertOwner(tenantId, userId);
...
export async function reactivateTenant(tenantId: string, userId: string) {
  await assertOwner(tenantId, userId);
  const tenant = await prisma.tenant.update({
    where: { id: tenantId },
...
export async function closeTenant(tenantId: string, userId: string) {
  await assertOwner(tenantId, userId);
  const tenant = await prisma.tenant.update({
    where: { id: tenantId },
    data: {
...
export async function getTenantPolicy(tenantId: string, userId: string) {
