...
export async function moveStock(tenantId: string, payload: any) {
  ...
  if (!product.isActive) throw new HttpError(400, 'Archived product cannot receive stock moves');

  const moves = await prisma.stockMove.findMany({ where: { tenantId, productId } });
  let currentQty = 0;
  for (const m of moves) currentQty += (m.type === 'IN' ? Number(m.quantity) : -Number(m.quantity));

  let currentAvg = product.costAvg != null ? Number(product.costAvg) : 0;
  let newAvg = currentAvg;

  if (type === 'IN') newAvg = computeNewAvgCost(currentQty, currentAvg, quantity, unitCost!);

  const move = await prisma.stockMove.create({
    data: { tenantId, productId, type: type as any, quantity: payload.quantity, unitCost: type === 'IN' ? payload.unitCost : null },
  });

  if (type === 'IN') await prisma.product.update({ where: { id: productId }, data: { costAvg: newAvg } });
...
export async function getBalance(tenantId: string) {
  ...
  for (const p of products) {
    ...
    let qty = 0;
    for (const m of moves) qty += (m.type === 'IN' ? Number(m.quantity) : -Number(m.quantity));
    out.push({
      productId: p.id,
      name: p.name,
      unit: p.unit.code,
