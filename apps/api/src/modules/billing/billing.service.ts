...
export function isPaidPlanUsable(status?: string | null) {
...
export function getEffectivePlan(
  ...
  if (!isPaidPlanUsable(billingStatus)) return 'FREE' as const;
  if (billingCurrentPeriodEnd && billingCurrentPeriodEnd.getTime() < Date.now()) return 'FREE' as const;
  return plan;
}

export async function expireTenantIfNeeded(tenantId: string) {
  ...
  if (!expiry) return tenant;
  if (!['ACTIVE', 'TRIALING', 'PENDING'].includes(String(tenant.billingStatus))) return tenant;
  if (expiry.getTime() >= Date.now()) return tenant;
...
function isPublicHttpUrl(url?: string | null) {
...
export async function getBillingStatus(tenantId: string) {
  ...
  const effectivePlan = getEffectivePlan(t.plan as any, t.billingStatus, t.billingCurrentPeriodEnd);
  return {
...
export async function startProTrial(opts: { tenantId: string }) {
  const tenant = await prisma.tenant.findUnique({ where: { id: opts.tenantId } });
  if (!tenant) throw new HttpError(404, 'Tenant not found');
  if (tenant.billingTrialUsedAt) throw new HttpError(409, 'Este tenant já usou o teste grátis do PRO');
  if (tenant.plan !== 'FREE') throw new HttpError(409, 'O teste grátis só pode começar a partir do plano FREE');
...
export async function createCheckoutSession(opts: { tenantId: string; userEmail: string; plan: 'PRO' | 'BUSINESS' }) {
  const tenant = await prisma.tenant.findUnique({ where: { id: opts.tenantId } });
  if (!tenant) throw new HttpError(404, 'Tenant not found');
  ...
  if (!checkoutUrl) throw new HttpError(500, 'Mercado Pago não retornou URL de checkout');
...
export async function createPortalSession(_opts: { tenantId: string; returnUrl?: string }) {
...
function mpPaymentStatusToInternal(status?: string | null) {
...
function mpPaymentStatusToBilling(status?: string | null) {
  switch (String(status || '').toLowerCase()) {
    case 'approved': return 'ACTIVE' as const;
    case 'cancelled': return 'CANCELED' as const;
    case 'rejected': return 'PAST_DUE' as const;
    case 'refunded': return 'PAST_DUE' as const;
    case 'charged_back': return 'PAST_DUE' as const;
    case 'expired': return 'EXPIRED' as const;
    default: return 'PENDING' as const;
  }
}

function parsePlanFromExternalReference(externalReference?: string | null): 'PRO' | 'BUSINESS' | null {
  if (!externalReference) return null;
  const m = /plan:(PRO|BUSINESS)/i.exec(externalReference);
  return (m?.[1]?.toUpperCase() as any) || null;
}

function parseTenantIdFromExternalReference(externalReference?: string | null): string | null {
  if (!externalReference) return null;
  const m = /tenant:([^:]+)/i.exec(externalReference);
  return m?.[1] || null;
}

function sortPaymentsByDateDesc(payments: any[] = []) {
...
function getMostRelevantMerchantOrderPayment(order: any) {
  const payments = sortPaymentsByDateDesc(order?.payments || []);
  return payments.find((p: any) => String(p.status || '').toLowerCase() === 'approved')
    || payments.find((p: any) => ['pending', 'in_process', 'authorized'].includes(String(p.status || '').toLowerCase()))
    || payments[0]
    || null;
}

export async function syncPaymentById(paymentId: string, webhookPayload?: any) {
  let payment: any;
  try {
  ...
  const plan = (payment.metadata?.plan || parsePlanFromExternalReference(externalReference)) as 'PRO' | 'BUSINESS' | null;
  if (!tenantId || !plan) throw new HttpError(400, 'Webhook do Mercado Pago sem tenantId/plan válidos');
  ...
  if (subscription) {
    const updated = await prisma.billingSubscription.update({
      ...
      data: {
        ...
        metadata: {
          ...(typeof subscription.metadata === 'object' && subscription.metadata ? subscription.metadata as any : {}),
...
export async function syncMerchantOrderById(orderId: string, webhookPayload?: any) {
...
export async function reconcileBillingStatus(tenantId: string) {
  await expireTenantIfNeeded(tenantId);
  const tenant = await prisma.tenant.findUnique({
    where: { id: tenantId },
    select: {
  ...
  const relevant = results.find((p: any) => String(p?.status || '').toLowerCase() === 'approved')
    || results.find((p: any) => ['pending', 'in_process', 'authorized'].includes(String(p?.status || '').toLowerCase()))
    || results[0]
    || null;
...
export async function cancelBilling(tenantId: string) {
  await prisma.tenant.update({
    where: { id: tenantId },
    data: {
      plan: 'FREE',
...
export async function assertProductCreationAllowed(tenantId: string) {
  await expireTenantIfNeeded(tenantId);
  const tenant = await prisma.tenant.findUnique({
    where: { id: tenantId },
  ...
  const effectivePlan = getEffectivePlan(tenant.plan as any, tenant.billingStatus, tenant.billingCurrentPeriodEnd);
