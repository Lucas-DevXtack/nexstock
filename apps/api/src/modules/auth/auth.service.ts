...
export async function updateMe(userId: string, input: { name?: string; avatarUrl?: string | null }) {
  const data: { name?: string; avatarUrl?: string | null } = {};

  if (typeof input.name === 'string') {
    const name = input.name.trim();
    if (name.length < 2) throw new HttpError(400, 'Nome inválido');
    data.name = name;
  }
  ...
  if (input.avatarUrl !== undefined) {
    if (input.avatarUrl === null || input.avatarUrl === '') {
      ...
      if (!avatarUrl.startsWith('data:image/')) throw new HttpError(400, 'Formato de imagem inválido');
      if (avatarUrl.length > 2_500_000) throw new HttpError(400, 'Imagem muito grande');
...
export async function changePassword(userId: string, currentPassword: string, newPassword: string) {
  if (!currentPassword || !newPassword) throw new HttpError(400, 'currentPassword and newPassword required');
  if (newPassword.length < 8) throw new HttpError(400, 'Password too short');
