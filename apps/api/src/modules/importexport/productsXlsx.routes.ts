...
// ------------------------------
// In-memory store (MVP)
// Em produção: Redis/DB.
// ------------------------------
type ImportError = { row: number; field?: string; code: string; message: string; value?: any };
type PreviewRow = {
  row: number;
  sku: string | null;
  name: string;
...
async function validateWorkbook(tenantId: string, buf: Buffer) {
  const wb = XLSX.read(buf, { type: 'buffer' });
  const sheetName = wb.SheetNames[0];
  if (!sheetName) throw new HttpError(400, 'xlsx has no sheets');
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    // validações
    if (!name) errors.push({ row: rowNum, field: 'name', code: 'REQUIRED', message: 'Obrigatório' });
    if (!unitCode) errors.push({ row: rowNum, field: 'unitCode', code: 'REQUIRED', message: 'Obrigatório (ex: un, kg, g, l, ml)' });
    if (unitCode && !unitMap.has(unitCode)) errors.push({ row: rowNum, field: 'unitCode', code: 'INVALID_UNIT', message: `Unidade inválida: ${unitCode}`, value: unitCode });

    if (minStock != null && minStock < 0) errors.push({ row: rowNum, field: 'minStock', code: 'INVALID_NUMBER', message: 'Deve ser >= 0', value: minStock });
    if (sellPrice != null && sellPrice < 0) errors.push({ row: rowNum, field: 'sellPrice', code: 'INVALID_NUMBER', message: 'Deve ser >= 0', value: sellPrice });

    if (initialQty != null && initialQty < 0) errors.push({ row: rowNum, field: 'initialQty', code: 'INVALID_NUMBER', message: 'Deve ser >= 0', value: initialQty });
    if (initialUnitCost != null && initialUnitCost < 0) errors.push({ row: rowNum, field: 'initialUnitCost', code: 'INVALID_NUMBER', message: 'Deve ser >= 0', value: initialUnitCost });
...
productsXlsxRoutes.post('/validate', async (req, res) => {
  try {
    ...
    if (!base64.trim()) throw new HttpError(400, 'base64 required');
...
productsXlsxRoutes.get('/errors.xlsx', async (req, res) => {
  try {
    ...
    if (!data) throw new HttpError(404, 'token not found/expired');
    if (data.tenantId !== req.tenantId) throw new HttpError(403, 'forbidden');

    const wsErr = XLSX.utils.json_to_sheet(
      data.errors.map(e => ({
        row: e.row,
        field: e.field || '',
        code: e.code,
...
productsXlsxRoutes.post('/commit', async (req, res) => {
  try {
    ...
    if (!importToken) throw new HttpError(400, 'importToken required');

    const data = store.get(importToken);
    if (!data) throw new HttpError(404, 'token not found/expired');
    if (data.tenantId !== req.tenantId) throw new HttpError(403, 'forbidden');
    if (data.summary.invalid > 0) throw new HttpError(400, 'Import has errors; fix before commit');
    ...
    await prisma.$transaction(async (tx) => {
      ...
      for (const r of data.preview) {
        ...
        if (!unit) throw new HttpError(400, `Invalid unit during commit: ${r.unitCode}`);
        ...
        if (r.category) {
          ...
          if (!catMap.has(key)) {
            const cat = await tx.productCategory.upsert({
              where: { tenantId_name: { tenantId: req.tenantId!, name: key } } as any,
              update: {},
              create: { tenantId: req.tenantId!, name: key } as any,
              select: { id: true },
            });
...
productsXlsxRoutes.post('/import', async (req, res) => {
  try {
    ...
    const buf = Buffer.from(base64.replace(/^data:.*?;base64,/, ''), 'base64');
    const { summary, preview, errors } = await validateWorkbook(req.tenantId!, buf);
    if (dryRun) return ok(res, { created: 0, updated: 0, errors, summary });

    if (summary.invalid > 0) return ok(res, { created: 0, updated: 0, errors, summary });
    ...
    const r = await (async () => {
      // reuse commit logic
      const fakeReq: any = { ...req, body: { importToken } };
      const fakeRes: any = {};
