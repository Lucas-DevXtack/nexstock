...
export async function moveStockFIFO(opts) {
    ...
    if (opts.type === 'ADJUST') {
        ...
        if (opts.unitCost == null)
            throw new HttpError(400, 'unitCost required for ADJUST');
        const cost = D(opts.unitCost);
        const lot = await prisma.stockLot.create({
            data: { tenantId: opts.tenantId, productId: opts.productId, unitCost: cost, qtyInitial: qty, qtyRemain: qty, batchCode: opts.batchCode },
        });
        const move = await prisma.stockMove.create({
            data: { tenantId: opts.tenantId, productId: opts.productId, type: 'ADJUST', reason: reason, quantity: qty, unitCost: cost, note: opts.note, occurredAt: opts.occurredAt ? new Date(opts.occurredAt) : undefined, createdByUserId: opts.userId },
        });
...
export async function getStockBalance(tenantId) {
    ...
    for (const lot of lots) {
        ...
        cur.immobilizedValue = cur.immobilizedValue.plus(lot.qtyRemain.mul(lot.unitCost));
        map.set(p.id, cur);
    }
