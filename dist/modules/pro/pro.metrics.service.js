...
function addDaysPortion(ms) {
    return ms / DAY_MS;
}
export async function productMetrics(tenantId, days = 30) {
    const now = new Date();
    ...
    for (const lot of lots) {
        ...
        cur.qty = cur.qty.plus(lot.qtyRemain);
        cur.value = cur.value.plus(lot.qtyRemain.mul(lot.unitCost));
    ...
    for (const m of moves) {
        ...
        if (m.type === 'OUT' && m.reason === 'SALE') {
            ...
            if (m.unitPrice)
                cur.revenue = cur.revenue.plus(m.unitPrice.mul(q));
            let cogs = D(0);
    ...
    const out = products.map((p) => {
        ...
        let ruptureDaysPct = null;
        const threshold = p.lowStockThreshold != null ? D(p.lowStockThreshold) : null;
        if (threshold) {
            ...
            for (const m of pmoves) {
                ...
                if (qty.lt(threshold))
                    belowDays += addDaysPortion(segMs);
                const q = m.quantity;
            ...
            if (qty.lt(threshold))
                belowDays += addDaysPortion(tailMs);
