...
export async function proHealth(tenantId, days = 30) {
    ...
    const weighted = metrics
        ...
        .reduce((acc, p) => acc + p.turnoverDays * p.immobilizedValue, 0);
    const immTotal = metrics
        .filter((p) => p.turnoverDays != null && (p.immobilizedValue || 0) > 0)
        .reduce((acc, p) => acc + p.immobilizedValue, 0);
    ...
    const idle = metrics
        ...
        .slice(0, 5);
    if (idle.length)
        recommendations.push(`Reduzir compra / limpar excesso: ${idle.map((x) => x.name).join(', ')}.`);
    const ruptureWorst = metrics
        .filter((p) => (p.ruptureDaysPct ?? 0) > 0)
        .sort((a, b) => (b.ruptureDaysPct || 0) - (a.ruptureDaysPct || 0))
        .slice(0, 5);
    if (ruptureWorst.length)
        recommendations.push(`Ajustar mínimo / repor: ${ruptureWorst.map((x) => `${x.name} (${Number(x.ruptureDaysPct).toFixed(1)}%)`).join(', ')}.`);
    const lowMargin = metrics
        .filter((p) => (p.revenue || 0) > 0)
        .sort((a, b) => (a.marginPct || 0) - (b.marginPct || 0))
        .slice(0, 5);
    if (lowMargin.length)
        recommendations.push(`Preço x custo apertado: ${lowMargin.map((x) => `${x.name} (${Number(x.marginPct).toFixed(1)}%)`).join(', ')}.`);
    let unlockEstimate = null;
