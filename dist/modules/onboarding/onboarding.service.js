...
export async function submitOnboarding(tenantId, payload) {
    ...
    if (!businessType || !regime)
        throw new HttpError(400, 'businessType and regime required');
...
export async function getOnboardingStatus(tenantId) {
    const [profile, tenant, productsCount, stockLotsCount, salesCount] = await Promise.all([
        ...
        prisma.stockMove.count({ where: { tenantId, type: 'OUT', reason: 'SALE' } }),
    ]);
    return {
