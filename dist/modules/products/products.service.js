...
async function assertUnitExists(unitId) {
    ...
    if (!unit)
        throw new HttpError(400, 'Invalid unitId');
}
export async function listProducts(tenantId, includeArchived = false) {
...
export async function createProduct(tenantId, payload) {
    ...
    if (!data.name || !data.unitId)
        throw new HttpError(400, 'name and unitId required');
...
export async function updateProduct(tenantId, productId, payload) {
...
export async function archiveProduct(tenantId, productId) {
    const existing = await prisma.product.findFirst({ where: { id: productId, tenantId } });
    if (!existing)
        throw new HttpError(404, 'Product not found');
    return prisma.product.update({
...
export async function unarchiveProduct(tenantId, productId) {
    const existing = await prisma.product.findFirst({ where: { id: productId, tenantId } });
    if (!existing)
        throw new HttpError(404, 'Product not found');
    return prisma.product.update({
