...
function pickNotification(input, query) {
...
function getProviderEventId(body, resourceId) {
    return String(body.id || resourceId || Date.now());
}
function isExpectedWebhookState(err) {
    const status = Number(err?.status || 0);
    const message = String(err?.message || '').toLowerCase();
    return ((status === 404 && message.includes('payment not found')) ||
